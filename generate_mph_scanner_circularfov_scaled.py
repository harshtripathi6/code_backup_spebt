...
def scale_count(base_count: int, scale_factor: float, *, multiple: int = 1) -> int:
    """Scale a discrete component count while preserving a required multiple."""
    if base_count <= 0:
        raise ValueError(f"base_count must be positive, got {base_count}")
    if scale_factor <= 0.0:
        raise ValueError(f"scale_factor must be positive, got {scale_factor}")
    if multiple <= 0:
        raise ValueError(f"multiple must be positive, got {multiple}")

    scaled = max(multiple, int(round(base_count * scale_factor)))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fov-diameter-mm",
        "--fov_diameter_mm",
        dest="fov_diameter_mm",
        type=float,
        default=10.0,
        help=(
            "Target FOV diameter in mm. Radial centerlines and default component counts "
            "are scaled relative to the 10 mm reference geometry (default: 10.0)."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Final aperture count. If omitted, scales the reference count of 180 "
            "by FOV/reference-FOV."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Additional uniform radial shift in mm applied after proportional scaling. "
            "Positive moves detector rings outward; negative moves inward."
        ),
    )
    parser.add_argument(
        "--skip-plot",
        action="store_true",
        help="Skip the detailed geometry plot, useful for very large detector counts.",
    )
    ...
    # ===== Reference geometry: original 10 mm FOV scanner =====
    REFERENCE_FOV_DIAMETER_MM = 10.0
    BASE_COLLIMATOR_INNER_DIAM_MM = 67.5
    BASE_COLLIMATOR_THICKNESS_MM = 2.5
    BASE_APERTURE_COUNT = 180
    BASE_DETECTOR_INNER_DIAMS_MM = [260.0, 390.0, 520.0, 650.0]
    BASE_DETS_PER_RING = [40*6*2, 40*9*2, 40*12*2, 40*15*2]
    BASE_SCINT_RADIAL_MM = 6.0

    FOV_DIAMETER_MM = cli_args.fov_diameter_mm
    if FOV_DIAMETER_MM <= 0.0:
        raise ValueError(f"FOV diameter must be positive, got {FOV_DIAMETER_MM}")
    SCALE_FACTOR = FOV_DIAMETER_MM / REFERENCE_FOV_DIAMETER_MM
    ...
    MIN_SPACING_MM = 0.8
    APERTURE_COUNT = (
        cli_args.n_apertures
        if cli_args.n_apertures is not None
        else scale_count(BASE_APERTURE_COUNT, SCALE_FACTOR)
    )
    if APERTURE_COUNT <= 0:
        raise ValueError(f"Aperture count must be positive, got {APERTURE_COUNT}")

    # Preserve the original collimator centerline-radius/FOV-radius ratio while
    # keeping the physical tungsten thickness unchanged.
    base_collimator_center_radius_mm = (
        BASE_COLLIMATOR_INNER_DIAM_MM / 2.0 + BASE_COLLIMATOR_THICKNESS_MM / 2.0
    )
    collimator_center_radius_mm = base_collimator_center_radius_mm * SCALE_FACTOR
    collimator_inner_radius_mm = collimator_center_radius_mm - RING_THICKNESS_MM / 2.0
    if collimator_inner_radius_mm <= 0.0:
        raise ValueError(
            f"Scaled collimator inner radius must be positive, got {collimator_inner_radius_mm:.3f} mm"
        )
    RING_INNER_DIAM_MM = 2.0 * collimator_inner_radius_mm
    ...
    # Preserve each original detector centerline-radius/FOV-radius ratio, while
    # retaining the original crystal dimensions.
    base_detector_center_radii_mm = [
        inner_d / 2.0 + BASE_SCINT_RADIAL_MM / 2.0
        for inner_d in BASE_DETECTOR_INNER_DIAMS_MM
    ]
    scaled_detector_center_radii_mm = [
        radius * SCALE_FACTOR for radius in base_detector_center_radii_mm
    ]
    RING_INNER_DIAMS_MM = [
        2.0 * (radius - SCINT_RADIAL_MM / 2.0)
        for radius in scaled_detector_center_radii_mm
    ]
    if any(inner_d <= 0.0 for inner_d in RING_INNER_DIAMS_MM):
        raise ValueError(f"Scaled detector inner diameters must be positive: {RING_INNER_DIAMS_MM}")

    # Scale detector CELLS, then multiply by two scintillators per cell.
    base_cells_per_ring = [count // 2 for count in BASE_DETS_PER_RING]
    cells_per_ring = [scale_count(count, SCALE_FACTOR) for count in base_cells_per_ring]
    DETS_PER_RING = [2 * count for count in cells_per_ring]

    print("Generating SC-SPECT base geometry")
    print(f"Reference FOV diameter: {REFERENCE_FOV_DIAMETER_MM:.3f} mm")
    print(f"Target FOV diameter:    {FOV_DIAMETER_MM:.3f} mm")
    print(f"Geometry scale factor:  {SCALE_FACTOR:.6f}x")
    print(f"Collimator center radius: {collimator_center_radius_mm:.3f} mm")
    print(f"Collimator inner diameter: {RING_INNER_DIAM_MM:.3f} mm")
    print(f"Aperture count: {APERTURE_COUNT}")
    print(f"Detector inner diameters: {[round(value, 3) for value in RING_INNER_DIAMS_MM]}")
    print(f"Detectors per ring: {DETS_PER_RING}")
    ...
    expected_total_scintillators = sum(DETS_PER_RING)
    print(
        f"Total scintillators: {geom_stats['total_scintillators']} "
        f"(expected {expected_total_scintillators})"
    )
    assert geom_stats["total_scintillators"] == expected_total_scintillators
    ...
    dtheta_open = APERTURE_DIAM_MM / r_center_ap  # approximate angular opening at ring centerline
    ...
    plate_segments = torch.stack(plate_segments_list, dim=0)  # (2*APERTURE_COUNT, 4, 2)
    ...
    out_data: OutDataDict = {  # type: ignore
        ...
        "paper_config": {
            "reference_fov_diameter_mm": REFERENCE_FOV_DIAMETER_MM,
    ...
    if cli_args.skip_plot:
        print("Skipping detailed geometry plot because --skip-plot was set")
    else:
        ...
        # Draw detectors (fixed). For the 150 mm FOV configuration this can be slow
        # because there are approximately 50,400 scintillators.
        if base_detector_units.numel() > 0:
            plot_polygons_from_vertices_2d_mpl(
                ...
                label=f"Detectors (Rings 1–4, {sum(DETS_PER_RING):,})"
            )
        ...
        ax.set_ylabel("Y (mm)")
        ax.set_title(
            f"SC-SPECT (Top View) — HR Ring with {APERTURE_COUNT:,} Apertures "
            f"(FOV {FOV_DIAMETER_MM:g} mm, Base Pose)"
        )
    ...
    print("n_positions:", n_total_positions)

    # Exact adjacent-center spacing for uniformly distributed aperture centers.
    dmin = (
        2.0 * r_center_ap * math.sin(math.pi / APERTURE_COUNT)
        if APERTURE_COUNT > 1
        else float("inf")
    )
