...
if __name__ == "__main__":
    ...
    parser.add_argument("--displacement", type=float, default=10.0)  # mm
    parser.add_argument("--run_id", type=str, required=True)
    parser.add_argument("--out_dir", type=str, default="runs")
    args = parser.parse_args()
    # --- 1. Define Base MPH-SPECT Configuration Parameters ---
    cfg_pinhole_diameter_mm = args.diameter
    cfg_n_pinholes = 24
    ...
    cfg_detector_outward_disp_mm = args.displacement  # random per-detector displacement in [0,2] mm
...
# Assuming helper.py is available
try:
    from helper import (
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--diameter", type=float, default=3.0)
    parser.add_argument("--displacement", type=float, default=10.0)
    parser.add_argument("--run_id", type=str, required=True)
    parser.add_argument("--out_dir", type=str, default="runs") # Added output dir
    args = parser.parse_args()
    ...
    # --- 1. Define Base MPH-SPECT Configuration Parameters ---
    cfg_pinhole_diameter_mm = args.diameter
    cfg_detector_outward_disp_mm = args.displacement
    
    # ... (Keep your constants same as before) ...
    cfg_n_pinholes = 24
    cfg_collimator_ring_radius_mm = 215.0
    cfg_pinhole_to_detector_distance_mm = 542.0
    cfg_scint_tangential_mm = 3.5
    cfg_scint_radial_thickness_mm = 6.0
    cfg_pinhole_channel_length_mm = 20.0
    cfg_detector_outward_ratio = 0.20
    cfg_detector_outward_seed = 123

    print(f"Generating base MPH-SPECT geometry (D={args.diameter}, Disp={args.displacement})...")
    
    base_detector_units, base_plate_segments = generate_mph_spect_geometry(
        pinhole_diameter_mm=cfg_pinhole_diameter_mm,
        n_pinholes=cfg_n_pinholes,
        collimator_ring_radius_mm=cfg_collimator_ring_radius_mm,
        pinhole_to_detector_distance_mm=cfg_pinhole_to_detector_distance_mm,
    ...
    n_rotations_for_motion = 15
    n_shifts_grid_for_motion = [1, 1]
    shift_step_mm_for_motion = [0, 0]

    motion_positions = positions_parameters(
        n_rotations_for_motion,
        360.0 / n_rotations_for_motion,
        n_shifts_grid_for_motion,
        shift_step_mm_for_motion
    )
    n_total_positions = motion_positions.shape[0]
    ...
    # --- 5. Save using RUN_ID in filename ---
    # This allows the next script to find it deterministically
    out_filename = f"scanner_layouts_{args.run_id}.tensor"
    out_path = os.path.join(args.out_dir, out_filename)
    
    print(f"\nSaving transformed layouts to: {out_path}")
    torch_save(out_data, out_path)
    print("Done.")