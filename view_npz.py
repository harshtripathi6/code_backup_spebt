...
npz_file_path = os.path.join(base_dir, "hotrod-projs-3mm-displaced-24ph.npz")
output_dir = os.path.join(base_dir, "hotrod_reconstruction_view_images18_3mm_displaced") # Directory to save output images
...
plt.imshow(final_recon, cmap='gray_r')
plt.colorbar(label="Image Intensity")
plt.title("Final Reconstructed Image")
plt.xlabel("Pixel")
plt.ylabel("Pixel")
# Save the figure
final_image_path = os.path.join(output_dir, "final_reconstructions18.png")
...
for i, (ax, idx) in enumerate(zip(axes, indices_to_show)):
    ax.imshow(reconstructions[idx], cmap='gray_r')
    ax.set_title(f"After ~Iteration {iteration_numbers[i]}")
    ax.set_xlabel("Pixel")
    ax.set_ylabel("Pixel")

# Save the comparison figure
comparison_image_path = os.path.join(output_dir, "comparison_reconstruction18.png")
...
try:
    ...
    im = ax_anim.imshow(reconstructions[0], cmap='gray_r', animated=True)
