...
if __name__ == "__main__":
    ...
    print(f"--- Starting Mask Extraction for {run_id} (Layout {layout_idx}) ---")
    ...
    # Output: Beam Masks
    out_hdf5_filename = f"beams_masks_{run_id}_layout_{layout_idx:03d}.hdf5"
    ...
    out_hdf5_file, beams_masks_dataset = initialize_beam_masks_hdf5(
        fov_n_pixels_int, out_hdf5_filename, work_dir
    )
    ...
    for detector_unit_idx in detector_units_sequence:
        ...
        # D. Combine Masks (e.g. Label Map or Combined Binary)
    ...
    # --- Finalize ---
    print(f"Layout {layout_idx} Final Shape: {list(beams_masks_dataset.shape)}")
    out_hdf5_file.close()
    
    print(f"--- Finished Mask Extraction for {run_id} ---")